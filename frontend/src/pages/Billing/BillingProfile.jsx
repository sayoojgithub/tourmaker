...
const BillingProfile = () => {
  const [tab, setTab] = useState("Confirmed Clients");
  ...
  useEffect(() => {
    const fetchBillingEmployeeDetails = async () => {
      try {
        const storedUser = JSON.parse(localStorage.getItem('user'));
        if (storedUser) {
          const response = await fetch(`${BASE_URL}/billing/billingEmployeeDetails/${storedUser._id}`, {
            headers: {
  ...
  const handleAddMore = (clientId) => {
    setSelectedClientId(clientId);
    setTab("Add Things"); 
  };
  const handleSeeLedger = (clientId) => {
    setSelectedClientId(clientId)
    setTab("Ledger Details")
  }
  const switchToBookedClients = () =>{
    setTab("Booked Clients")
  }
      ...
      <div className="pb-[30px] px-[20px] rounded-md">
          <div
            className={`flex items-center justify-center ${
              tab === "Confirmed Clients" ? "mt-[130px]" :tab === "Client Details" ?"mt-[110px]":tab === "Booked Clients" ? "mt-[130px]" :tab === "Add Things" ? "mt-[100px]": "mt-[110px]"
            }`}
          >
            <img
        ...
        <div className="md:col-span-2 md:px-[30px] m-2">
          <div className="flex flex-col md:flex-row space-y-1 md:space-y-0 md:space-x-1 ml-1">
            <button
              type="button"
              className={buttonClasses("Confirmed Clients")}
              onClick={() => setTab("Confirmed Clients")}
            >
              Confirmed Clients
            </button>
            {/* <button
            ...
            {/* <button
              type="button"
              className={buttonClasses("Ledger Details")}
              //onClick={() => setTab("Ledger Details")}
            >
              Ledger Details
            </button> */}
            <button
          ...
          <div className="mt-4 md:mt-4 mb-1">
            {tab === "Confirmed Clients" && (
              <div><ConfirmedClientsTable onSeeMore={handleSeeMore} onAddMore={handleAddMore}/></div>
            )}
            {tab === "Client Details" && (
              <div><ClientDetails clientId={selectedClientId}  switchToBookedClients={switchToBookedClients}/></div>
            )}
            {tab === "Booked Clients" && <div><BookedClientsTable onSeeMore={handleSeeMore} onSeeLedger={handleSeeLedger}/></div>}
            {tab === "Ledger Details" && (
              <div><LedgerDetails clientId={selectedClientId}/></div>
            )}
            {tab === "Add Things" && (
              <div><ItemEntryTable clientId={selectedClientId}/></div>
            )}
            {tab === "Report" && (
