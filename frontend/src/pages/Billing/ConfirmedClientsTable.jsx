...
const ConfirmedClientsTable = ({ onSeeMore, onAddMore }) => {
  ...
  const fetchClients = async (term = searchTerm) => {
    ...
    try {
      ...
      const queryParams = new URLSearchParams({
        ...
        companyId,
        ...(term &&
          (isNaN(term) ? { clientId: term } : { mobileNumber: term })), // Dynamically add clientId or mobileNumber based on search term type
      });

      console.log("Query Params:", queryParams.toString()); // Debugging line

      const response = await fetch(
        `${BASE_URL}/billing/confirmed-clients?${queryParams}`
      );
      if (!response.ok) {
  ...
  return (
    ...
    <div className="flex items-center gap-1 bg-white/30 px-4 py-1 rounded-lg shadow">
      <h2 className="text-2xl font-semibold mb-1 text-center text-red-600">Confirmed Clients</h2>
    </div>
    </div>
        ...
        <table className="w-full border border-gray-200 text-left rounded-lg">
          <thead className="bg-gray-400">
            <tr>
              ...
              <th className="px-4 py-2 border-b font-extrabold">Add</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
                    ...
                    <span
                      ...
                      onClick={() => {
                        if (!client.invoiceNumber) {
                          onAddMore(client._id);
                        }
                      }}
                      className={
                        client.invoiceNumber ? "pointer-events-none" : ""
                      }
                    >
                      ➕
                    </span>
...
export default ConfirmedClientsTable;
