...
const CompanyCustomerCares = ({onEdit}) => {
      ...
      try {
        ...
        if (company && company._id) {
          ...
          const response = await fetch(
            `${BASE_URL}/company/customerCaresList?companyId=${companyId}&page=${currentPage}&limit=4&search=${searchQuery}`
          );

          if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || "Failed to fetch customer cares list");
          }

          const result = await response.json();
          setCustomerCares(result.data);
          setTotalPages(result.totalPages);
        } else {
          console.error("User ID not found in localStorage");
        }
      } catch (error) {
        console.error("Failed to fetch customer cares list:", error.message);
      }
    };
  ...
  <div className="flex items-center gap-1 bg-white/30 px-2 py-2 rounded-lg shadow">
    <h2 className="text-xl font-semibold text-red-600">CustomerCares List</h2>
  </div>
</div>
      ...
      <div className="flex mb-4">
        <input
          ...
          placeholder="Search by customer care name"
        ...
        <table className="w-full border-collapse border border-white/30 rounded-lg overflow-hidden">
          ...
          <tbody>
            {customerCares.map((customerCare) => (
              <tr key={customerCare._id} className="text-gray-800">
                <td className="border border-white/30 px-4 py-2 text-center">{customerCare.name}</td>
                <td className="border border-white/30 px-4 py-2 text-center">{customerCare.mobileNumber}</td>
                <td className="border border-white/30 px-4 py-2 text-center">{customerCare.email}</td>
                <td className="border border-white/30 px-4 py-2 text-center">
                  <button
                    onClick={() => onEdit(customerCare)}
...
export default CompanyCustomerCares;