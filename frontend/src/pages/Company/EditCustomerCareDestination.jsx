...
const EditCustomerCareDestinations = ({ customercare, onBack }) => {
  ...
  useEffect(() => {
    const fetchDestinations = async () => {
      try {
        const response = await fetch(
          `${BASE_URL}/company/customercareDestinations/${customercare._id}?page=${page}&search=${searchQuery}`
        );
    ...
    fetchDestinations();
  }, [customercare._id, page, searchQuery, refreshFlag]); // Fetch when page or search changes
  useEffect(() => {
  ...
  if (isAlreadyAdded) {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/company/addCustomerCareDestination/${customercare._id}`,
        {
          method: "POST",
          headers: {
  ...
  const handleDelete = async (destinationId) => {
    try {
      const response = await fetch(
        `${BASE_URL}/company/deleteCustomerCareDestinations/${customercare._id}/${destinationId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  <div className="flex items-center gap-1 bg-white/30 px-2 py-2 rounded-lg shadow">
    <h2 className="text-xl font-semibold text-red-600">
      {customercare.name}'s Assigned Destinations
    </h2>
  </div>
</div>
...
export default EditCustomerCareDestinations;
