...
const PendingClientsTable = ({ onSeeMore }) => {
  ...
  const fetchClients = async (term = searchTerm) => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/customercare/pending-clients?${queryParams}`
      );
      if (!response.ok) {
  ...
  return (
    <div className="p-6 bg-white/20 shadow-2xl rounded-lg mt-4">
      ...
      <h2 className="text-2xl font-semibold mb-1 text-black">Pending Clients</h2>
      </div>
      </div>

      {/* Search Section */}
      {/* Search Section */}
      <div className="mb-6 bg-white/20 shadow-md rounded-xl p-3 flex flex-col md:flex-row items-center gap-4">
        {/* Search Input */}
            ...
            {loading ? (
              ...
              clients.map((client) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 border-b font-bold">
                    {new Date(client.finalizedTourDate).toLocaleDateString(
                      "en-GB"
                    )}
                  </td>
                  <td className="px-4 py-2 border-b font-bold">
