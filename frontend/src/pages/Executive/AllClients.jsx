  ...
  const fetchClients = async (term = searchTerm) => {
    ...
    try {
      const queryParams = new URLSearchParams({
        executiveId: userId,
        page: currentPage,
        ...(term && (isNaN(term) ? { clientId: term } : { mobileNumber: term })),
      });

      console.log("Query Params:", queryParams.toString()); // Debugging line

      const response = await fetch(`${BASE_URL}/executive/all-clients?${queryParams}`);
      if (!response.ok) {
  ...
  return (
    <div className="p-6 bg-white/20 shadow-2xl rounded-lg mt-2 w-full max-w-[90vw] mx-auto">
      ...
      <div className="flex items-center gap-1 bg-white/30 px-4 py-2 rounded-lg shadow">
        <h2 className="text-2xl font-semibold text-red-600">All Clients</h2>
        {/* If you want to show count here, uncomment below */}
        {/* <span className="ml-2 text-xl text-gray-700 font-bold">({totalClients})</span> */}
      </div>
    </div>
            ...
            {loading ? (
              ...
              clients.map((client) => (
                <tr key={client._id} className="hover:bg-white/20">
                  ...
                  <td className="px-4 py-2 border-b font-bold">{client.status}</td>
