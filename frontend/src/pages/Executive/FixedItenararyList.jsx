...
const FixedItenararyList = ({ selectedClientId, selectedPrimaryTourName, handleSeeMoreOfFixedTrip }) => {
  ...
  const fetchFixedTours = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/executive/fixed-itineraries?${queryParams}`
      );
      if (!response.ok) {
  ...
  return (
    <div className="p-6 bg-white/20 shadow-md rounded-lg mt-2 w-full max-w-[90vw] mx-auto">
      <h2 className="text-2xl font-semibold mb-4 text-center">GROUP TOURS</h2>
        ...
        <table className="w-full border border-gray-200 text-left rounded-lg">
          <thead className="bg-gray-400">
            <tr>
              ...
              <th className="px-4 py-2 border-b font-extrabold">Total Pax</th>
              <th className="px-4 py-2 border-b font-extrabold">MRP</th>
            ...
            {loading ? (
              ...
              tours.map((tour) => (
                <tr key={tour._id} className="hover:bg-white/20">
                  ...
                  <td className="px-4 py-2 border-b font-bold">{tour.totalPax || "-"}</td>
                  <td className="px-4 py-2 border-b font-bold">{tour.mrp || "-"}</td>
                  <td className="px-4 py-2 border-b font-bold">
                    {tour.category?.label || "-"}
                  </td>
                  <td className="px-4 py-2 border-b text-blue-500 hover:underline cursor-pointer">
                    <span title="See More" onClick={() => handleSeeMoreOfFixedTrip(tour._id)}>➕</span>
                  </td>
                </tr>
              ))
            ) : (
...
export default FixedItenararyList;
