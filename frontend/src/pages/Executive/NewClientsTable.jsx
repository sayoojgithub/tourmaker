...
const NewClientsTable = ({ onSeeMore }) => {
  ...
  const fetchClients = async (term = searchTerm) => {
    ...
    try {
      const queryParams = new URLSearchParams({
        executiveId: userId,
        page: currentPage,
        clientId: term, // Only clientId for search
      });

      console.log("Query Params:", queryParams.toString()); // Debugging line

      const response = await fetch(`${BASE_URL}/executive/new-clients?${queryParams}`);
      if (!response.ok) {
  ...
  return (
    <div className="p-6 bg-white/20 shadow-2xl rounded-lg mt-1 w-full max-w-[90vw] mx-auto">
      ...
      <h2 className="text-2xl font-semibold text-red-600">New Clients</h2>
            ...
            {loading ? (
              ...
              clients.map((client) => (
                <tr key={client._id} className="hover:bg-white/20">
                  ...
                  <td className="px-4 py-2 border-b font-bold text-blue-500 hover:underline cursor-pointer">
