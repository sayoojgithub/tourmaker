...
const ToDoTable = ({ onSeeMore }) => {
  ...
  const fetchClients = async (term = searchTerm) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/executive/todo-clients?${queryParams}`);
      if (!response.ok) {
  ...
  return (
    <div className="p-6 bg-white/20 shadow-2xl rounded-lg mt-1 w-full max-w-[90vw] mx-auto">
      ...
      <h2 className="text-2xl font-semibold text-red-600">Todo Clients</h2>
