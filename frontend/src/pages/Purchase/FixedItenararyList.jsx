...
const FixedItineraryList = ({ onEdit }) => {
  const [fixedTrips, setFixedTrips] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const [searchQuery, setSearchQuery] = useState("");

  useEffect(() => {
    const fetchFixedTrips = async () => {
      try {
        ...
        if (user && user._id) {
          ...
          const response = await fetch(
            `${BASE_URL}/purchaser/fixedTrips?purchaserId=${purchaserId}&page=${currentPage}&limit=5&search=${searchQuery}`
          );

          if (!response.ok) {
            const errorData = await response.json();
            throw new Error(
              errorData.message || "Failed to fetch itineraries"
            );
          }

          const result = await response.json();
          setFixedTrips(result.data);
  ...
  <div className="inline-block bg-white px-3 py-1 rounded-md shadow">
    <h2 className="text-xl font-bold text-red-600 text-center">
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr>
            <th className="border px-8 py-1 font-extrabold">Destination</th>
            <th className="border px-8 py-1 font-extrabold">Tour Name</th>
            <th className="border px-4 py-1 font-extrabold">Article Number</th>
            <th className="border px-4 py-1 font-extrabold">Category</th>
            <th className="border px-4 py-1 font-extrabold">Actions</th>
          </tr>
        </thead>
        <tbody>
          {fixedTrips.map((fixedtrip) => (
            <tr key={fixedtrip._id}>
              <td className="border px-8 py-1 font-bold text-center">
                {fixedtrip.destination.value}
              </td>
              <td className="border px-8 py-1 font-bold text-center">
                {fixedtrip.tourName}
              </td>
              <td className="border px-4 py-1 font-bold text-center">
                {fixedtrip.articleNumber}
              </td>
              <td className="border px-4 py-1 font-bold text-center">
                {fixedtrip.category.value}
              </td>
              <td className="border px-4 py-1">
                <button
                  onClick={() => handleEdit(fixedtrip._id)}
...
export default FixedItineraryList;
