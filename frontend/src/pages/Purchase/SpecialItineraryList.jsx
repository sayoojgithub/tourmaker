...
const SpecialItineraryList = ({ onEdit }) => {
      ...
      try {
        ...
        if (user && user._id) {
          ...
          const response = await fetch(
            `${BASE_URL}/purchaser/specialTrips?purchaserId=${purchaserId}&page=${currentPage}&limit=5&search=${searchQuery}`
          );
  ...
  <div className="inline-block bg-white px-3 py-1 rounded-md shadow">
    <h2 className="text-xl font-bold text-red-600 text-center">
      ...
      <table className="w-full border-collapse">
        ...
        <tbody>
          {specialTrips.map((specialtrip) => (
            <tr key={specialtrip._id}>
              <td className="border px-8 py-1 font-bold text-center">
                {specialtrip.destination.value}
              </td>
              <td className="border px-8 py-1 font-bold text-center">
                {specialtrip.tourName}
              </td>
              <td className="border px-4 py-1 font-bold text-center">
                {specialtrip.articleNumber}
              </td>
              <td className="border px-4 py-1 font-bold text-center">
                {specialtrip.category.value}
              </td>
              <td className="border px-4 py-1">
                <button
                  onClick={() => handleEdit(specialtrip._id)}
...
export default SpecialItineraryList;
