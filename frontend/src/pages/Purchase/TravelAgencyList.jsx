      ...
      try {
        ...
        if (user && user._id) {
          ...
          const response = await fetch(
            `${BASE_URL}/purchaser/travelAgencies?purchaserId=${purchaserId}&page=${currentPage}&limit=5&search=${searchQuery}`
          );

          if (!response.ok) {
            const errorData = await response.json();
            throw new Error(
              errorData.message || "Failed to fetch transportations"
            );
          }

          const result = await response.json();
          setTravelAgencies(result.data);
          setTotalPages(result.totalPages);
        } else {
          console.error("User ID not found in localStorage");
        }
      } catch (error) {
        console.error("Failed to fetch transportations:", error.message);
      }
    };
    ...
    <h2 className="text-xl font-bold text-red-600 text-center">
      ...
      <div className="flex mb-4">
        <input
          ...
          placeholder="Search by travelAgency name or ID or Destination name..."
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr>
            ...
            <th className="border px-4 py-1 font-extrabold">PhoneNumber</th>
        ...
        <tbody>
          {travelAgencies.map((travelAgency) => (
            <tr key={travelAgency._id}>
              ...
              <td className="border px-4 py-1 font-bold text-center">
                {travelAgency.travelsName}
              </td>
              <td className="border px-4 py-1 font-bold text-center">
                {travelAgency.destinationName}
              </td>

              <td className="border px-4 py-1 font-bold text-center">
                {travelAgency.contactNumber
                  ? travelAgency.contactNumber
                  : "Not available"}
              </td>
              <td className="border px-4 py-1">
                <button
                  onClick={() => handleEdit(travelAgency._id)}
