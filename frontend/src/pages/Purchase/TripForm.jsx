...
const TripForm = ({ onNext, tripId }) => {
  ...
  useEffect(() => {
    if (tripId) {
      const fetchTripDetails = async () => {
        try {
          const response = await fetch(`${BASE_URL}/purchaser/trip/${tripId}`);
          if (!response.ok) {
  ...
  const fetchDestinations = async () => {
    try {
      ...
      setLoading((prevLoading) => ({ ...prevLoading, destination: false }));
    } catch (error) {
  ...
  // Function to fetch travelagencies from the backend
  ...
  const handleAddTravelDetail = () => {
    const { selectedAgency, selectedCategory, selectedVehicle,selectedVehicleId, price } =
      formData;
  ...
  const handleAddTrip = async () => {
    const { destinationName,destinationId, tripName, tripDescription, travelsDetails } =
      formData;
    ...
    try {
      ...
      if (tripId) {
        ...
        const response = await fetch(
          `${BASE_URL}/purchaser/updateTrip/${tripId}`,
          {
            method: "PUT",
            headers: {
        ...
        // Add new trip
        const response = await fetch(`${BASE_URL}/purchaser/addTrip`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  // useEffect to log the trips whenever it updates
  useEffect(() => {
  ...
  const handleSelectChange = (selectedOption, field) => {
    setFormData((prev) => {
      // Check if the field is 'selectedVehicle'
  ...
  console.log(formData, "after selecting everything");
