...
const ClientsOfExecutive = ({ executiveId, onSeeMore }) => {
 const [clients, setClients] = useState([]);
   ...
   const fetchClients = async () => {
     ...
     try {
       ...
       const response = await fetch(
         `${BASE_URL}/salesManager/all-clients-of-executive?${queryParams}`
       );
       if (!response.ok) {
   ...
   const handleStatusChange = (e) => {
    ...
    setCurrentPage(1); // Reset to page 1 when the status changes
  };
   ...
   const handlePrevPage = () => {
    ...
    // Extract executiveName dynamically from the first client object
  const executiveName = clients.length > 0 ? clients[0].executiveName : "Executive";
 
   return (
     <div className="p-6 bg-white/20 shadow-2xl rounded-lg mt-2">
       <h2 className="text-2xl font-semibold mb-2 text-center">Clients of {executiveName}</h2>
      ...
      <div className="mb-4 flex items-center gap-4">
        ...
        <button
          onClick={fetchClients} // Fetch clients based on the selected status
...
export default ClientsOfExecutive